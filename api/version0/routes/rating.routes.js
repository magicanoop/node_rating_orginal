...
const rating = require("../controllers/rating.controler");
const validate = require('express-jsonschema').validate;  
const { createRatingSchema } = require('../validationSchemas/ratingValidationSchema');  
...
router.use('/', function (req, res, next) {
    ...
    req.currentModule = "rating";
    
    next();
});

// Create a new user
router.post("/", validate({ body: createRatingSchema }), rating.create);
router.get("/getrating", rating.getrating);
router.get("/getpostrating/:postid", rating.getpostrating);
router.post("/update/:id", rating.updaterating);
